...
train_img_names = sorted(os.listdir(train_cache))
train_img_paths = [train_cache + t for t in train_img_names]

test_img_names = sorted(os.listdir(test_cache))
...
def train_nn(iteration, lr, epochs, images_num, training_sample_num, test_sample_num, model_file_name, eval_file_name, model, train_func):
    ...
    for i in range(iteration):
        ...
        # normal samples
        images = choose_random_images(images_num, train_img_paths)
        data_chunk = gen_data(images, training_sample_num)

        result = train_func(model, data_chunk, epochs)
...
def train(iteration=5, lr=0.01, epochs=1, memory=0.5, images_num=1, training_sample_num=16, test_sample_num=8):
    ...
    train_nn(iteration, lr, epochs, images_num, training_sample_num, test_sample_num, model_file_name, eval_file_name, model, nn.train_batch)
...
def test(lr=0.01, memory=0.5):
    test_nn(lr, memory)


def predict(model_name="model", memory=0.5):
...
def transfer(tfr_img_id=0, model_name="model", memory=0.5):
    # Set the chosen image 
    # - the test and train is on the same image
    # - the one on which the user is working 
    temp = test_img_paths[tfr_img_id:tfr_img_id+3]
    test_img_paths = temp
